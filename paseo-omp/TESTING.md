...
| Terminal ownership on later turns | **Version-gated** | A new exact-text branch entry proves user-message persistence, not ownership of an unkeyed `agent_end`. The plugin accepts a later-turn terminal when it carries the originating prompt's RPC `requestId`. Older OMP releases remain fail-closed after the ownership deadline unless the off-by-default `PASEO_OMP_LEGACY_TERMINAL_OWNERSHIP=correlated-user` escape hatch documented in [SUPPORT.md](SUPPORT.md#legacy-terminal-ownership-opt-in) is enabled. Regressions cover matching and mismatched request IDs, stale ends before and after new entries, repeated prompts, evidence arriving during an older terminal check, unavailable or hung history, buffered legacy evidence, correlation that finishes after a terminal, mismatched direct IDs, invalid and revalidated watermarks, unrecognized values, and child-environment exclusion. The real-OMP matrix reuses one runtime for three legacy-owned turns. |
